...
ReNJobWindow::ReNJobWindow() : RelionJobWindow(1, HAS_MPI, HAS_NOT_THREAD) 
{
	tab1->begin();
	tab1->label("I/O");
	resetHeight();
	whole_particles.place(
                      current_y,
                      "Whole particles:",
                      "",
                      "Input particles(*.{star})",
                      "");
        renumber_name.place(current_y, "Renumber name :", "renumber",
                              "Suffix of renumber particles");
	tab1->end();

	read(".gui_renumber_img.settings", is_continue);

}


void ReNJobWindow::write(std::string fn)
{
	std::ofstream fh;
	openWriteFile(fn + ".gui_renumber_img.settings", fh);

	whole_particles.writeValue(fh);
        renumber_name.writeValue(fh);

	closeWriteFile(fh);
	
}

void ReNJobWindow::read(std::string fn, bool &_is_continue)
{
	std::ifstream fh;
        if (openReadFile(fn, fh))
        {
	whole_particles.readValue(fh);
        renumber_name.readValue(fh);
 	closeReadFile(fh);
        _is_continue = is_continue;
	}
}

void ReNJobWindow::toggle_new_continue(bool _is_continue)
{
	is_continue = _is_continue;
}
void ReNJobWindow::getCommands(std::string &outputname, std::vector<std::string> &commands,
		std::string &final_command, double angpix, double particle_diameter)
{
	commands.clear();
	std::string command;
	if (nr_mpi.getValue() > 1)
		command="`which relion_subtract_micrograph_mpi`";
	else
		command="`which relion_subtract_micrograph`";

	//I/O
	if(whole_particles.getValue() != "") command += " --input_star " + whole_particles.getValue();
	if(renumber_name.getValue() != "") command += " --renumber_name " + renumber_name.getValue();
	command += " --renumber_rawimages ";  
...
//subtract micrograph
SubJobWindow::SubJobWindow() : RelionJobWindow(4, HAS_MPI, HAS_NOT_THREAD) 
{
	tab1->begin();
	tab1->label("I/O");
	resetHeight();
      	whole_particles.place(
                      ...
                      "",
                      "Input particles(*.{star})",
                      "Whole particles or raw particles data.star");
	pixel_size.place(current_y,
                                "Pixel size of orignal micrograph:", 1, 1, 3, 0.01,
                                "Pixel size");
	current_y += STEPY / 2;
	reference.place(
                      current_y,
                      ...
                      "",
                      "Star file(*.{star})",
                      "The corresponding reference(s) of whole particles or subtracting particles if specifying subtracting particles.");
	mask.place(
                      current_y,
                      "Mask:",
                      "",
                      "Input mask(*.{mrc})",
                      "Specifying the region to be projected and subtracted.");
	low_pass_filter.place(current_y,
                                "Low pass filter(A):", -1, 0, 60, 1,
                                "Low pass filter");
	binning_factor_of_reference.place(current_y,
                                "Binning factor of references :", 1, 1, 10, 1,
                                "Binning factor of reference(s), whole and subtracting particles.");
	invert_contrast_of_references.place( current_y,
                              "Invert contrast of references?",
                              false,
                              ""
                              );
        invert_micrographs.place( current_y,
                              "Invert contrast of micrographs?",
                              false,
                              ""
                              );
	...
	ctf_intact_first_peak.place( current_y,
	...
	//Subtract micrograph or neighbor
	tab3->begin();
 	tab3->label("Subtract micrograph or neighbor");
	resetHeight();
	subtract_reference_group = new Fl_Group(WCOL0,  MENUHEIGHT, 550, 600-MENUHEIGHT, "");
        subtract_reference_group->end();	
	subtract_neighbor_group = new Fl_Group(WCOL0,  MENUHEIGHT, 550, 600-MENUHEIGHT, "");
        subtract_neighbor_group->end();
	subtract_neighbor_group->begin();
	subtract_reference.place( current_y,
                              "Subtract ref. from micrographs?",
                              false,
                              "",
		    	      subtract_reference_group,1
			       //reverse bool type	
                              );
	output_subtracted.place( current_y,
                              "Output subtracted micrographs",
                              false,
                              "Output intermediate micrographs after subtracting projections of reference(s)"
                              );
	output_projection.place( current_y,
                              "Output projection micrographs",
                              false,
                              "Output intermediate micrographs containing projections of reference(s)"
                              );
 	subtract_neighbor_group->end();	
	current_y += STEPY / 2;
	subtract_reference_group->begin();
	subtract_neighbor.place( current_y,
                              "Subtract neighbor particles",
                              false,
                              "",
			      subtract_neighbor_group,1
			       //reverse bool type
                              );
	
	pre_extraction.place(current_y,
                                "Neighbor size(even number):", 1024, 0, 8192, 2,
                                "Box size of intermediate image");
	subtract_reference_group->end();
	...
	extract_name.place(current_y, "Suffix for rewindowed particles:", "rewindow",
                              "");
	extract_size.place(current_y,
                                "Size of the box to extract particles", -1, -1, 1000, 1,
                                "");
	current_y += STEPY / 2;
	norm.place( current_y,
				"Normalise the background?",
				true,
				""
				 );
	new_particle_diameter.place(current_y,
                                "New particle diameter", 100, 0, 1000, 1,
                                "in Angstrom");
	white_dust.place(current_y,
                                "White dust remove:", -1, -1, 10, 1,
                                "");
        black_dust.place(current_y,
                                "Black dust remove:", -1, -1, 10, 1,
                                "");
	current_y += STEPY / 2;

	target_xyz_group = new Fl_Group(WCOL0,  MENUHEIGHT, 550, 600-MENUHEIGHT, "");
	target_xyz_group->end();
	coordinate_group = new Fl_Group(WCOL0,  MENUHEIGHT, 550, 600-MENUHEIGHT, "");
        coordinate_group->end();
	
	coordinate_group->begin();
	coordinate_of_particles.place(
                      current_y,
                      "Coordinate of particles: ",
		      target_xyz_group,
                      "",
                      "Star file(*.{star})",
                      "According to whose coordinates the sub-particles are re-windowed.");
	coordinate_group->end();

	target_xyz_group->begin();
	target_x.place(current_y,
                                "Coordinate X of target", 0, 0, 8192, 1,
                                "Relative to the volume center",
				coordinate_group);
	target_y.place(current_y,
                                "Coordinate Y of target", 0, 0, 8192, 1,
                                "Relative to the volume center",
                                coordinate_group);
	target_z.place(current_y,
                                "Coordinate Z of target", 0, 0, 8192, 1,
                                "Relative to the volume center",
                                coordinate_group);
	target_xyz_group->end();

	subtract_neighbor.set_deactivated_groups(coordinate_group);	
	subtract_neighbor.set_deactivated_groups(target_xyz_group);


	tab4->end();

	// read settings if hidden file exists
	read(".gui_subtract_mic.settings", is_continue);
	subtract_reference.cb_menu_i_rev();
	subtract_neighbor.cb_menu_i_rev();
	coordinate_of_particles.cb_input_i();
        target_x.cb_input_i();
        target_y.cb_input_i();
        target_z.cb_input_i();
}
...
void SubJobWindow::write(std::string fn)
{
	std::ofstream fh;
	openWriteFile(fn + ".gui_subtract_mic.settings", fh);

	whole_particles.writeValue(fh);
	coordinate_of_particles.writeValue(fh);
	reference.writeValue(fh);
	mask.writeValue(fh);
	pixel_size.writeValue(fh);
	binning_factor_of_reference.writeValue(fh);
	low_pass_filter.writeValue(fh);
	renumber_rawimages.writeValue(fh);
	rewindow_micrograph.writeValue(fh);
	renumber_name.writeValue(fh);
	invert_contrast_of_references.writeValue(fh);
	invert_micrographs.writeValue(fh);
	ctf.writeValue(fh);
	ctf_intact_first_peak.writeValue(fh);
	subtract_reference.writeValue(fh);
	output_subtracted.writeValue(fh);
	output_projection.writeValue(fh);
        subtract_neighbor.writeValue(fh);
        extract.writeValue(fh);
        norm.writeValue(fh);
        extract_name.writeValue(fh);
        new_particle_diameter.writeValue(fh);
        extract_size.writeValue(fh);
        white_dust.writeValue(fh);
        black_dust.writeValue(fh);
        target_x.writeValue(fh);
        target_y.writeValue(fh);
        target_z.writeValue(fh);
	closeWriteFile(fh);
}

void SubJobWindow::read(std::string fn, bool &_is_continue)
{
	std::ifstream fh;
	if (openReadFile(fn, fh))
	{
	whole_particles.readValue(fh);
        coordinate_of_particles.readValue(fh);
        reference.readValue(fh);
        mask.readValue(fh);
        pixel_size.readValue(fh);
        binning_factor_of_reference.readValue(fh);
        low_pass_filter.readValue(fh);
        renumber_rawimages.readValue(fh);
        rewindow_micrograph.readValue(fh);
        renumber_name.readValue(fh);
        invert_contrast_of_references.readValue(fh);
        invert_micrographs.readValue(fh);
        ctf.readValue(fh);
        ctf_intact_first_peak.readValue(fh);
        subtract_reference.readValue(fh);
        output_subtracted.readValue(fh);
	output_projection.readValue(fh);
        subtract_neighbor.readValue(fh);
        extract.readValue(fh);
        norm.readValue(fh);
        extract_name.readValue(fh);
        new_particle_diameter.readValue(fh);
        extract_size.readValue(fh);
        white_dust.readValue(fh);
        black_dust.readValue(fh);
        target_x.readValue(fh);
        target_y.readValue(fh);
        target_z.readValue(fh);
	closeReadFile(fh);
	_is_continue = is_continue;
	}
}

void SubJobWindow::toggle_new_continue(bool _is_continue)
{
	is_continue = _is_continue;
}
void SubJobWindow::getCommands(std::string &outputname, std::vector<std::string> &commands,
		std::string &final_command, double angpix, double particle_diameter)
{
	commands.clear();
	std::string command;
	...
	if(whole_particles.getValue() != "") 
		command += " --input_star " + whole_particles.getValue();
	if(reference.getValue() != "") command += " --ref " + reference.getValue();
	command += " --angpix_micrograph " + floatToString(pixel_size.getValue());
	if(mask.getValue() != "") command += " --mask " + mask.getValue();
	if(low_pass_filter.getValue() >= 0) command += " --lowpass " + floatToString(low_pass_filter.getValue());
	command += " --binning " + floatToString(binning_factor_of_reference.getValue());
	if(invert_contrast_of_references.getValue()) command += " --invert_refs ";
	command += " --rewindow_micrograph ";  
	if(invert_micrographs.getValue()) command += " --invert_micrographs ";


	//CTF
	command += " --apply_ctf_to_total_projection ";
	if(ctf.getValue()) command += " --ctf ";
	if(ctf_intact_first_peak.getValue()) command += " --ctf_intact_first_peak ";


	//Subtract Micrograph or neighbor
	if(!subtract_neighbor.getValue()){
		if(subtract_reference.getValue()) command += " --subtract ";
		if(output_subtracted.getValue()) command += " --output_refsub_micrographs ";
		if(output_projection.getValue()) command += " --output_projection_micrographs ";
	}
	if(!subtract_reference.getValue()){
		if(subtract_neighbor.getValue()) {
		command += " --subtract_neighbor ";
		command += " --neighbor_size " + floatToString(pre_extraction.getValue());
		}
	}
	...
	if(extract.getValue()) {
		command += " --reextract ";
		command += " --extract_name " + extract_name.getValue();
		command += " --extract_size " + floatToString(extract_size.getValue());
	}
	if(norm.getValue()) command += " --norm ";
	command += " --new_particle_diameter " + floatToString(new_particle_diameter.getValue());
	if(white_dust.getValue() >= 0 ) command += " --white_dust " + floatToString(white_dust.getValue());
	if(black_dust.getValue() >= 0 ) command += " --black_dust " + floatToString(black_dust.getValue());
	if(!subtract_neighbor.getValue())
	if(coordinate_of_particles.getValue() == ""){
                if(target_x.getValue() != 0 ) command += " --target_x " + floatToString(target_x.getValue());
                if(target_y.getValue() != 0 ) command += " --target_y " + floatToString(target_y.getValue());
                if(target_z.getValue() != 0 ) command += " --target_z " + floatToString(target_z.getValue());

        }else{
                command += " --coord_star " + coordinate_of_particles.getValue();
        }
...
//subtract particles
SupJobWindow::SupJobWindow() : RelionJobWindow(3, HAS_MPI, HAS_NOT_THREAD) 
{
	tab1->begin();
	tab1->label("I/O");
	resetHeight();
      	whole_particles.place(
                      ...
                      "Whole particles:",
                      "",
                      "Particles to be subtracted(*.{star})",
                      "Whole particles or raw particles data.star");
	pixel_size.place(current_y,
                                "Pixel size of original micrograph:", 1, 1, 3, 0.01,
                                "Pixel size");
	current_y += STEPY / 2;
	subtracting_particles.place(
                      current_y,
                      "Subtracting particles(optional):",
                      "",
                      "Subtracting particles(*.{star})",
                      "data.star of particles according to whose Euler angles the reference(s) is(are) projected and subtracted from whole or raw particles");
	reference.place(
                      current_y,
                      "Reference: ",
                      "",
	...
	//Subtract particles
	tab3->begin();
 	tab3->label("Subtract and/or reextract");
	resetHeight();
	
	subtract_reference.place( current_y,
                              "Subtract reference from particles?",
                              false,
                              ""
                              );
	output_particles.place( current_y,
                              "Output raw particles",
                              true,
                              "Output matching raw particles."
                              );
        output_sub_particles.place( current_y,
                              "Output subtracting particles",
                              true,
                              "Output matching subtracting particles"
                              );
	extract_name.place(current_y, "Suffix for rewindowed particles:", "rewindow",
                              "");
	extract_size.place(current_y,
                                "Size of the box to extract particles", 100, -1, 1000, 1,
                                "");
	current_y += STEPY / 2;
	 norm.place( current_y,
                                "Normalise the background?",
	...
	// read settings if hidden file exists
	read(".gui_subtract_ptl.settings", is_continue);
        coordinate_of_particles.cb_input_i();
        target_x.cb_input_i();
        target_y.cb_input_i();
        target_z.cb_input_i();	
...
void SupJobWindow::write(std::string fn)
{
	std::ofstream fh;
	openWriteFile(fn + ".gui_subtract_ptl.settings", fh);

	whole_particles.writeValue(fh);
	coordinate_of_particles.writeValue(fh);
	reference.writeValue(fh);
	mask.writeValue(fh);
	subtracting_particles.writeValue(fh);
	pixel_size.writeValue(fh);
	binning_factor_of_reference.writeValue(fh);
	low_pass_filter.writeValue(fh);
	renumber_rawimages.writeValue(fh);
	...
	ctf_intact_first_peak.writeValue(fh);
	subtract_reference.writeValue(fh);
	output_particles.writeValue(fh);
	output_sub_particles.writeValue(fh);
	norm.writeValue(fh);
	extract_name.writeValue(fh);
	pre_extraction.writeValue(fh);
	extract_size.writeValue(fh);
	new_particle_diameter.writeValue(fh);
	white_dust.writeValue(fh);
	black_dust.writeValue(fh);
...
void SupJobWindow::read(std::string fn, bool &_is_continue)
{
	std::ifstream fh;
	if (openReadFile(fn, fh))
	{
	whole_particles.readValue(fh);
        ...
        reference.readValue(fh);
        mask.readValue(fh);
        subtracting_particles.readValue(fh);
        pixel_size.readValue(fh);
        binning_factor_of_reference.readValue(fh);
        low_pass_filter.readValue(fh);
        renumber_rawimages.readValue(fh);
        ...
        ctf_intact_first_peak.readValue(fh);
        subtract_reference.readValue(fh);
        output_particles.readValue(fh);
        output_sub_particles.readValue(fh);
        norm.readValue(fh);
        extract_name.readValue(fh);
        pre_extraction.readValue(fh);
        extract_size.readValue(fh);
        new_particle_diameter.readValue(fh);
        white_dust.readValue(fh);
        black_dust.readValue(fh);
...
void SupJobWindow::toggle_new_continue(bool _is_continue)
{
	is_continue = _is_continue;
}
void SupJobWindow::getCommands(std::string &outputname, std::vector<std::string> &commands,
		std::string &final_command, double angpix, double particle_diameter)
{
	commands.clear();
	std::string command;
	...
	if(whole_particles.getValue() != "") command += " --input_star " + whole_particles.getValue();
        if(reference.getValue() != "") command += " --ref " + reference.getValue();
	command += " --angpix_micrograph " + floatToString(pixel_size.getValue());
	if(subtracting_particles.getValue() != "") command += " --sub_star " + subtracting_particles.getValue();
        if(mask.getValue() != "") command += " --mask " + mask.getValue();
        if(low_pass_filter.getValue() >= 0) command += " --lowpass " + floatToString(low_pass_filter.getValue());
        command += " --binning " + floatToString(binning_factor_of_reference.getValue());
        if(invert_contrast_of_references.getValue()) command += " --invert_refs ";
	command += " --rewindow_particle ";
	...
	//Subtract particles
        if(!output_particles.getValue()) command += " --do_not_output_raw_particles ";
        if(!output_sub_particles.getValue()) command += " --do_not_output_sub_particles ";
