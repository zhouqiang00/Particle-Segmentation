...
void FileNameEntry::place(int &y,
                                ...
                                const char* helptext,
                                int x , int h, int wcol2, int wcol3, int wcol4 ){
	place(y, title, defaultvalue, pattern, helptext, x, h, wcol2, wcol3, wcol4);
	my_deactivate_group_ = deactivate_this_group;	
}
...
void FileNameEntry::cb_browse_i() {
    ...
    inp->value(relname);
    if(this->getValue() != ""){
	if(my_deactivate_group_ != NULL)
		my_deactivate_group_->deactivate();
    }
}

void FileNameEntry::cb_input(Fl_Widget* o, void* v) {
...
void FileNameEntry::cb_input_i() {
    if(this->getValue() == ""){
	if(my_deactivate_group_ != NULL)
                my_deactivate_group_->activate();
    }else{
	if(my_deactivate_group_ != NULL)
                my_deactivate_group_->deactivate();
    }
		    				 ...
		    				 Fl_Group * deactivate_this_group)
{
    ...
    //type for different deactivator
    type_ = 0;
}
...
void RadioEntry::cb_menu_i_rev(){
	...
	for(int i =0; i < my_deactivate_groups.size();i++)
        if (strcmp(inp->value(), "Yes") == 0){
                my_deactivate_groups[i]->deactivate();
        }else{
                my_deactivate_groups[i]->activate();
	}
}
...
void BooleanEntry::place(int &y,
		...
		bool defaultvalue,
		const char* helptext,
		Fl_Group * deactivate_this_group, int type,
		int x, int h, int wcol2, int wcol3, int wcol4 )
{
...
void SliderEntry::place(int &y,
	...
	place(y, title, defaultvalue, minvalue, maxvalue, valuestep, help, x, h, wcol2, wcol3, wcol4);
	my_deactivate_group_ = my_deactivate_group;
...
void SliderEntry::cb_slider_i() {
    ...
    if(this->getValue() != defaultvalue_){
	if(my_deactivate_group_ != NULL)
		my_deactivate_group_->deactivate();	
    }else{
	if(my_deactivate_group_ != NULL)
