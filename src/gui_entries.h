...
public:
    ...
    std::vector<Fl_Group*> my_deactivate_groups;
...
public: // this one is public so that it can be called in mainwindow to deactivate default groups
    ...
    void cb_menu_i();
    void cb_menu_i_rev();
    //type = 0 no->deactivate; = 1 yes->deactivate
    int type_;

    void set_deactivated_groups(Fl_Group * deactivate_this_group){
	my_deactivate_groups.push_back(deactivate_this_group);
    }
};
...
public:
    ...
    void place(int &y,
				...
				const char* help = NULL, 
				Fl_Group * deactivate_this_group = NULL,int type = 0,
				int x = XCOL1, int h = STEPY, int wcol2 = WCOL2, int wcol3 = WCOL3, int wcol4 = WCOL4 );
