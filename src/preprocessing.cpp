...
void Preprocessing::read(int argc, char **argv, int rank)
{
	...
	// CTF correction etc. Modified by ZhouQ
	int corrections_section = parser.addSection("Corrections");
	do_ctf = parser.checkOption("--ctf", "Perform CTF correction when operate_on?");
	...
	only_flip_phases = parser.checkOption("--flip_phases", "Perform CTF phase-flipping on images when operate_on?");
...
void Preprocessing::usage()
{
	// Modified by ZhouQ
	// for Update info
	std::cout << "Update 2016/01/05" << std::endl;
...
void Preprocessing::runOperateOnInputFile(FileName fn_operate_on)
{
	...
	for (long int i = 0; i < Nimg; i++)
	{
		...
		if (fn_operate_on.isStarFile())
		{
			...
			Ipart.read(fn_tmp);
			// Modifief by ZhouQ, in order to output phase-flipped particle images
			if (do_ctf && only_flip_phases)
			{
				CTF ctf;
				ctf.read(MD, MD, i);
				transformer.FourierTransform(Ipart(), Fimg);
				Fctf.resize(Fimg);
				ctf.getFftwImage(Fctf, XSIZE(Ipart()), YSIZE(Ipart()), angpix, false, only_flip_phases, false, false);
				FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY(Fimg)
				{
					DIRECT_MULTIDIM_ELEM(Fimg, n) *= DIRECT_MULTIDIM_ELEM(Fctf, n);
				}
				transformer.inverseFourierTransform(Fimg, Ipart());
				//CenterFFT(Ipart(), true);
			}
