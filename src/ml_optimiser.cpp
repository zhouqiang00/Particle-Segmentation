...
//#define DEBUG
	...
	if (do_map )
	{
		...
		// for invariant_ref, Modified by ZhouQ
		// set max resolution arbitrarily to the initial lowpass filter resolution
		if (do_invariantref)
		{
			if (ini_high > 0.)
			{
...
void MlOptimiser::updateImageSizeAndResolutionPointers()
{
	...
	// for invariant_ref, Modified by ZhouQ
	// if do invariant ref, skip updating maxres
	if ( ! do_invariantref)
	{
