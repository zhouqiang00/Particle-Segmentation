...
RelionMainWindow::RelionMainWindow(int w, int h, const char* title):Fl_Window(w,h,title)
{
    ...
    browser->add("Publish!");
    browser->add("Renumber images");
    browser->add("Subtract micrograph");
    browser->add("Subtract particles");
    ...
    {
	browse_grp[13] = new Fl_Group(WCOL0, MENUHEIGHT, 550, 600-MENUHEIGHT);
        job_ren = new ReNJobWindow();
        browse_grp[13]->end();	
    }
    {
	browse_grp[14] = new Fl_Group(WCOL0, MENUHEIGHT, 550, 600-MENUHEIGHT);
        job_sub = new SubJobWindow();
        browse_grp[14]->end();

   }
   {
        browse_grp[15] = new Fl_Group(WCOL0, MENUHEIGHT, 550, 600-MENUHEIGHT);
        job_sup = new SupJobWindow();
        browse_grp[15]->end();
...
void RelionMainWindow::jobCommunicate(bool do_write, bool do_read, bool do_toggle_continue, bool do_commandline, int this_job)
{
	...
	else if (myval == 14)
	{
		...
		if (do_write)
			job_ren->write(fn_settings);
		if (do_read)
			job_ren->read(fn_settings, is_main_continue);
		if (do_toggle_continue)
			job_ren->toggle_new_continue(is_main_continue);
		if (do_commandline)
			job_ren->getCommands(outputname, commands, final_command,
					job_general->angpix.getValue(),
					job_general->particle_diameter.getValue());
	...
	else if (myval == 15)
	{
		...
		if (do_write)
			job_sub->write(fn_settings);
		if (do_read)
			job_sub->read(fn_settings, is_main_continue);
		if (do_toggle_continue)
			job_sub->toggle_new_continue(is_main_continue);
		if (do_commandline)
			job_sub->getCommands(outputname, commands, final_command,
					job_general->angpix.getValue(),
					job_general->particle_diameter.getValue());
	...
	{
		if (do_write)
                        job_sup->write(fn_settings);
                if (do_read)
                        job_sup->read(fn_settings, is_main_continue);
                if (do_toggle_continue)
                        job_sup->toggle_new_continue(is_main_continue);
                if (do_commandline)
                        job_sup->getCommands(outputname, commands, final_command,
                                        job_general->angpix.getValue(),
                                        job_general->particle_diameter.getValue());
...
void RelionMainWindow::cb_menubar_load_i()
{
    ...
    int npos = fn_settings.length();
    	if (fn_settings.rfind(".gui_general.") < npos)
    		...
    		browser->value(12);
	else if  (fn_settings.rfind(".gui_renumber_img.") < npos)
    		browser->value(14);
	else if  (fn_settings.rfind(".gui_subtract_mic.") < npos)
    		browser->value(15);
	else if  (fn_settings.rfind(".gui_subtract_ptl.") < npos)
    		browser->value(16);
